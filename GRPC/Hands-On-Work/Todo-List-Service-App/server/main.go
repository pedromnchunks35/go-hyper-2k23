...
func (taskManager *Task) CreateTask(ctx context.Context, newTask *t.Task) (*t.Confirmation, error) {
	//? Checking the data
	if newTask == nil || newTask.Description == "" || newTask.EndDate == "" || newTask.StartDate == "" {
	...
	confirmation.Msg = fmt.Sprintf("The received task was saved with id: %v", taskManager.counter)
	taskManager.counter++
